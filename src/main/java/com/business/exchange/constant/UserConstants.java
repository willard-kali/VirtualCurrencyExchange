package com.business.exchange.constant;

public class UserConstants {
    public static final String DEFAULT_PWD = "Changeme123";

    public static final String CREATE_SUCCESS = "success";

    public static final String VALID_SUCCESS = "success";

    public static final String PWD_SUCCESS = "success";

    public static final String CREATE_FAILED = "failed";

    public static final String VALID_FAILED = "failed";

    public static final String PWD_FAILED = "failed";

    public static final int USERNAME_MAX_LENGTH = 100;

    public static final int EMPLOYEE_ID_MAX_LENGTH = 20;
}
